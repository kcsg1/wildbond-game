# Wildbond 아키텍처 v0.3 (2D · 싱글플레이 · Java 단일 스택)
...
| **v0.3** | **멀티플레이 제외.** net/server/platform 계층 삭제, sim은 클라이언트 프로세스 안에서 고정 틱으로 실행. 멀티는 §6 "확장 경로"로만 남김 |
...
| 영역 | 선택 | 이유 |
|---|---|---|
| 언어·런타임 | Java 21, jlink 런타임 번들 | record·sealed·패턴 매칭 |
| 클라이언트 | LibGDX 1.13 + LWJGL3, Scene2D.ui, steamworks4j | D-01 |
...
| 빌드·검증 | Gradle 8 Kotlin DSL, 테스트 하네스(`harness.sh`) | 버전 관리·CI는 후순위 |
...
| Player | 레벨, 기술 포인트, 스탯 배분, 인벤토리, 장비, 파티(팰 5), 위치, 도감 | 동적 |
...
| 모듈 | 내용 | 의존 |
|---|---|---|
| client-desktop | LWJGL3 런처, Steam 초기화, jlink/jpackage | client-core |
...
| 배포 | jlink 최소 런타임 + jpackage → Steam 빌드(Win/Linux/macOS) |
...
```
Google Sheets → data/tables/*.csv → datagen → data/src/generated/*.java (record)
                                           → assets/data/tables.bin (FlatBuffers)
검증: 참조 무결성, 값 범위, 중복 ID, 풋프린트 ≤ 4×4 → 빌드 실패 처리
```

| 테이블 | 키 컬럼 (발췌) |
|---|---|
| PalSpecies | id, name_key, element1, element2, base_hp, base_atk, base_def, work[10], partner_skill, footprint(1\|2), rideable, ride_speed, capture_rate, exp_yield, sprite_atlas |
...
| 영역 | 설계 |
|---|---|
| 월드 크기 | 초기 1024×1024 타일(32×32 청크). 청크 좌표 int → 확장 가능. 바이옴 6종 |
| 맵 제작 | Tiled → chunk-compiler. 프로시저럴(D-06)을 택하면 같은 .wbc로 출력 |
...
단일 Gradle 멀티프로젝트 `wildbond-game`. 모든 검증은 루트의 **테스트 하네스** `./harness.sh`를 통한다 (`CLAUDE.md` "테스트 하네스" 참고).
...
버전 관리(git)와 CI(GitLab, Trivy, SBOM)는 현재 범위 밖이며 나중에 도입한다. 도입 시 CI는 로컬과 같은 `./harness.sh`를 그대로 실행한다.
...
| M3 클로즈드 알파 | 3주 | Steam Cloud · 도전과제 · 크래시 로그 · 밸런스 1차 · 성능 예산 검증 · jpackage 배포 |
