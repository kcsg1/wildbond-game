...
## 2026-09-08 — 아트 방향 전환: Kenney CC0 에셋 도입
- 목표: "바람의나라처럼 이쁘게" 요청. 절차적으로 그린 도형으로는 손그림 아트에 닿을 수 없으므로, 사람과 상의해
  **무료 CC0 에셋 팩 도입**으로 방향을 잡았다.
- 한 일:
  - 에셋: Kenney **Tiny Town**(지형·나무) + **Tiny Dungeon**(캐릭터·팰) 도입. 둘 다 CC0 라 상업적 사용·수정이
    자유롭다 — 로드맵 §7 Steam 출시를 감안해 CC-BY-SA 류(LPC 등)는 일부러 피했다. `assets/CREDITS.md` 에 출처 기재
  - `PlaceholderTilesetGenerator` 가 Tiny Town 시트에서 지형 타일을 골라 32×32 로 확대해 합성한다.
    **물은 그 팩에 없어서 직접 그린다** — 이어 붙여도 줄무늬가 생기지 않게 세로 그라데이션을 쓰지 않았다
  - 타일셋을 5칸 → 12칸으로 늘렸다: 앞 5칸은 Tile.csv id 1~5, 뒤 7칸은 **렌더 전용**(잔디 변형 3 + 물가 경계 4).
    지형 종류가 아니라 "같은 지형을 덜 밋밋하게 그리는 수단"이라 Tile.csv 에 넣지 않았다
  - `ChunkRenderer`: 좌표 해시로 잔디 변형을 섞고(돌·꽃은 드물게), 물 타일에서 이웃이 물이 아닌 방향에만
    물가 띠를 덧그려 호수 가장자리가 직각으로 끊기지 않게 했다
  - `PlaceholderSprites` 를 두 시트에서 잘라 쓰는 구조로 바꿨다 — 플레이어·팰 3종·나무·바위·허수아비.
    팩에 없는 포획구와 그림자만 코드로 그린다
  - 나무 71그루를 Tiled 오브젝트로 배치하고, 컴파일러가 그 타일에 solid 충돌을 찍는다(테스트 추가)
- 검증: **하네스 전체 모드 통과** (`reports\harness60908-215438.md`, 68/0/0/0).
  `:client-desktop:run` 을 `PrintWindow`(T-007)로 캡처해 잔디 결·나무 숲·호수(물가 띠 포함)·캐릭터·팰을 확인했다.
  호수를 보려고 스폰을 잠시 연못 옆으로 옮겨 찍고 원위치했다
- 결정:
  - **CC0 만 쓴다.** 표기 의무가 없고 상업적 사용이 자유롭다. 전염성 라이선스는 나중에 되돌리기 어렵다
  - **16×16 원화를 32px 월드 타일로 2배 확대해 쓴다.** 픽셀이 굵어지지만 화면 전체가 같은 배율이라 일관된다.
    sim 의 32px 타일 규약(§4.1)을 건드리지 않는 쪽을 택했다
  - **Kenney 캐릭터에는 걷기 프레임이 없다.** 4방향 애니메이션은 좌우 반전 + 1px 들썩임으로 줄였다 —
    스프라이트 품질을 얻는 대신 감수한 부분이다
- 남은 일:
  - 물가 경계가 **청크 안에서만** 이웃을 본다 — 청크 경계에 걸친 물가는 띠가 빠진다(테스트 맵에서는 안 걸린다)
  - 모서리(대각) 경계 타일이 없어 물가가 각져 보인다. 오토타일 47종까지 가면 훨씬 매끄러워진다
  - 절벽·모래는 아직 Tiny Town 의 대체 타일이라 어색하다

## 2026-09-08 (2) — 바람의나라식 구조 전환: 존 3개 · HP/MP · 스페이스바 전투 · 전리품

- 목표: 마을에서 시작해 가장자리로 나가면 다음 맵, 굴에 들어가면 지정 몬스터. HP/MP 상태창, 스페이스바 공격,
  몬스터 머리 위 HP, 쓰러지면 동전 드롭 → 획득.
- **문서 먼저 고쳤다** (CLAUDE.md "설계를 바꾸려면 문서를 먼저"):
  - **D-16 존 분할 + 가장자리 전환** — §10 의 "연속된 1024² 월드 청크 스트리밍"을 뒤집는 결정이다.
    존 하나 = 독립 청크 집합 + 자체 스폰 규칙. 청크 포맷(§8.2)은 그대로고 폴더만 존별로 나뉜다
  - **D-17 스페이스바 근접 공격** — 단계 6 의 마우스 조준을 기본 조작에서 내렸다(원거리는 우클릭 유지)
  - §3.1 엔티티 표에 Zone·DroppedItem 추가, Player 에 HP/MP·소지금 명시
- 한 일 (맵):
  - `village.tmx`(집 8채·울타리·십자 흙길), `field.tmx`(연못·절벽·숲·굴 입구 포털),
    `cave.tmx`(방 6개 + 통로, 스폰 포인트 10곳) 생성. 존마다 `assets/maps/chunks/<zone>/` 로 컴파일
  - ChunkCompiler: 나무·바위뿐 아니라 **집(3×4)·울타리도 차지하는 칸 전부**를 solid 로 찍는다
- 한 일 (sim):
  - 컴포넌트 `Mana`·`Wallet`·`DroppedItem`·`DeathAnim`
  - `SpawnRules` — 존별 스폰 종·마리 수·최소 거리·스폰 포인트 사용 여부. 마을은 스폰 없음, 굴은 종 3만
  - `DropSystem` — 팰이 쓰러지면 그 자리에 동전, 플레이어가 다가가면 획득(`ItemDropped`/`CoinsPicked`)
  - MP: 투사체 스킬만 소모하고 초당 회복. 근접 기본 공격은 무료
  - SimView 에 `mana`/`maxMana`/`coins`/`dropAmount`/`deathProgress` 추가
- 한 일 (client):
  - `world/Zone`·`ZoneLink`·`ZoneRuntime` — 가장자리·포털을 보고 sim 을 새 존으로 다시 만든다
  - `StatusHud` — HP/MP 막대, 존 이름, 소지금
  - `EntityRenderer` — 몬스터 머리 위 HP 바(다쳤을 때만), 쓰러지는 연출(납작해지며 사라짐), 동전(위아래 흔들림),
    집·울타리 프롭
  - 스페이스바 = 바라보는 방향 근접 공격. 방향은 렌더가 이미 들고 있는 facing 을 쓴다 — sim 에 facing 을
    추가하지 않았다(§6 규칙 3 유지)
- 검증: **하네스 전체 모드 통과** (`reports/harness/20260908-221853.md`, 68/0/0/0).
  `PrintWindow`(T-007) 캡처로 확인: 마을 시작(집 4채·십자 길·HP 100/100·MP 50/50·coin 0),
  VILLAGE→FIELD 가장자리 전환, 굴(회색 바위 몬스터만 등장, HUD 에 CAVE). 굴 확인은 시작 존을 잠시 CAVE 로
  바꿔 찍고 되돌렸다
- 결정:
  - **존 전환은 sim 을 새로 만드는 것**이다. 넘어가며 유지되는 것은 소지금뿐이고 HP/MP 는 회복된다 —
    "플레이어 상태를 지정한다"는 명령이 M0 에 없어서, sim 상태를 밖에서 직접 고치지 않는다는 §6 규칙을
    지키는 쪽을 택했다. 제대로 실어 나르는 것은 세이브 포맷(§8.3)이 생기는 M1 몫이다
  - **머리 위 HP 바는 다친 몬스터만** 그린다. 전부 그리면 화면이 바로 지저분해진다
  - **동전은 아직 Item.csv 에 없다** — `DropSystem.DropConstants.COIN_ITEM_ID = 0` 자리표시자다.
    화폐를 아이템 표에 넣을지(스택·아이콘) 별도 재화로 둘지 정해지면 그때 채운다
- 남은 일:
  - **사람 확인 필요**: 스페이스바 공격 → 몬스터 HP 바 감소 → 쓰러짐 → 동전 → 획득까지의 흐름.
    이 환경은 입력 주입이 막혀 있다(T-007)
  - 굴 바닥이 Kenney 흙 타일이라 잔풀이 섞여 나온다 — 굴 전용 바닥 타일이 필요하다
  - 존 전환 시 파티 팰이 따라오지 않는다(새 sim 이라 사라진다). M1 세이브에서 함께 옮긴다
  - `ZoneRuntime.checkTransition` 이 매 프레임 포털 청크를 다시 읽는다(디스크+zstd). 청크 캐시를 두면 좋다

## 2026-09-09 — 플레이어 캐릭터 LPC 교체: 4방향 + 무기 휘두르기

- 요청: "유니티랑 연동해서 이쁘게, 무기 들고 스페이스바로 휘두르고, 이동 방향을 바라보게"
- **유니티는 도입하지 않기로 했다.** sim·클라이언트가 전부 Java/LibGDX(D-01)라 "연동"이 성립하지 않는다 —
  붙이려면 (a) 유니티 클라이언트 + Java sim IPC 브리지, (b) 전면 C# 재작성 둘 중 하나이고, 요청한 동작
  (4방향·공격 애니메이션) 중 유니티가 있어야만 되는 것은 하나도 없다. 사람과 상의해 **지금 구조 유지**로 확정.
  막고 있던 것은 엔진이 아니라 **캐릭터 스프라이트시트**였다 — Kenney 캐릭터는 정면 한 장뿐이었다.
- **D-18 아트 라이선스 완화** — 사람이 "스팀에 안 올린다, 개인용"으로 범위를 정해 CC-BY-SA/GPL 계열(LPC)을
  허용했다. 되돌릴 지점과 표기 의무는 architecture.md D-18 · assets/CREDITS.md 참고
- 한 일:
  - LPC 생성기 저장소에서 레이어(몸·머리·머리카락·상의·바지·신발·검)를 받아 두 장으로 합성:
    `lpc_player_walk.png`(4방향 × 9프레임), `lpc_player_slash.png`(4방향 × 6프레임).
    **둘 다 128×128 프레임**으로 통일했다 — 베기는 검이 캐릭터 밖으로 크게 나가 64칸에 안 들어간다
  - `PlaceholderSprites`: LPC 시트 로드 + 행 순서 변환(LPC 는 위/왼/아래/오른, 이 코드는 아래/위/오른/왼)
  - `EntityRenderer`: 걷기 9프레임 순환(정지 시 0번), `startSlash`/`isSlashing` 으로 베기 6프레임 재생.
    발 위치가 프레임 위에서 92px 지점이라 그만큼 내려 그린다
  - `PlayScreen`: 스페이스바에서 공격 명령과 베기 연출을 같이 시작
  - LPC 저작자 12명 표기를 assets/CREDITS.md 에 기록(CC-BY-SA·OGA-BY 의 의무)
- 검증: **하네스 전체 모드 통과** (`reports/harness/20260909-041703.md`, 68/0/0/0).
  `PrintWindow`(T-007) 캡처로 마을에서 칼 든 캐릭터가 집·길과 함께 정상 렌더되는 것 확인
- 결정:
  - **애니메이션 상태는 전부 렌더 쪽에 둔다.** 방향은 이미 이동량에서 뽑고 있었고, 베기도 PlayScreen 이
    입력을 감지한 프레임에 렌더러에 알린다 — sim 에 facing/attack 상태를 넣지 않아 §6 규칙 3 이 그대로 산다
  - **프레임 크기를 128 로 통일**했다. 걷기만 64 로 두면 앵커·크기 계산이 두 갈래가 된다
- 남은 일:
  - **사람 확인 필요**: 스페이스바 휘두르기와 4방향 전환은 입력이 필요해 확인 못 했다(T-007)
  - LPC 캐릭터(정교한 셰이딩)와 Kenney 지형(16px 청키)의 **화풍이 다르다**. 맞추려면 지형도 LPC 로
    옮기는 것이 정공법이다 — LPC 지형 타일은 32px 라 지금 월드 타일과 정확히 맞는다
  - 팰·허수아비는 아직 Kenney 그대로다
  - 옷 색이 기본 흰색이라 밋밋하다. LPC 팔레트로 다시 칠할 수 있다
