...
- 대상: M0 수직 슬라이스 (약 3주, 싱글플레이, Windows x64, Java 25)
- 전제: `docs/architecture.md` v0.4, `CLAUDE.md`
- 이 파일의 각 프롬프트는 Claude Code(또는 다른 코딩 에이전트)에 그대로 붙여 넣는다.

## 사용 방법

1. 한 단계 = 한 세션. 프롬프트를 붙여 넣고, 끝나면 **검증 명령**을 직접 실행해 확인한 뒤 다음 단계로 간다.
   - 에이전트는 `CLAUDE.md` "작업 절차"에 따라 시작 전 `troubleshooting.md`를 읽고, 끝나면 하네스 전체 모드를 통과시키고 `plan.md`에 히스토리를, 실패는 `troubleshooting.md`에 기록해야 한다. 기록이나 하네스 통과가 빠져 있으면 다음 단계로 가기 전에 요구한다.
...
```powershell
winget install Tiled.Tiled      # 단계 4 에서 필요, 지금은 없어도 됨
```

**검증**: `java -version` → 25.x, 64-Bit Server VM (설치 완료). 그 외 설치할 것 없음 —
하네스는 PowerShell 로 돌고, 버전 관리(git)는 나중에 도입한다.

**환경 변수** (한 번만, 새 셸부터 적용):

```powershell
[Environment]::SetEnvironmentVariable("GRADLE_USER_HOME", "C:\Users\user\Desktop\develop\.gradle-home", "User")
```

프로젝트와 Gradle 홈을 백신 예외 경로(바탕화면 트리)에 두기 위한 것이다 — troubleshooting.md T-002 참고.
...
요구사항:
- Gradle 9.x Kotlin DSL, version catalog(gradle/libs.versions.toml), Java 25 toolchain (D-15), Gradle wrapper 포함
  (gradlew.bat 로 Windows 에서 실행). 개발 PC 의 JDK 는 Temurin 25.0.1 하나뿐이니 toolchain 자동 다운로드에 의존하지 말 것
...
- 루트의 harness.ps1 이 실제로 돌게 만들기: spotlessCheck 와 build 태스크가 존재해야 하고, 모든 모듈의 test 태스크가
  JUnit XML 을 build\test-results\test\ 에 남겨야 함. 완료 조건은 .\harness.cmd 가 종료 코드 0 으로 끝나는 것

완료 후 .\harness.cmd 를 실행해 통과를 확인하고 리포트를 요약해줘.
```

### 검증

```
.\harness.cmd                      # 종료 코드 0, reports\harness\latest.md 생성
.\gradlew.bat :sim:test --tests "*ArchitectureTest*"
```
...
```
.\gradlew.bat :data:generateData :data:test
```
...
```
.\harness.cmd         # build + sim:bench 포함. 리플레이 테스트 통과, 벤치 평균 틱 ≤ 8ms
```
...
```
.\gradlew.bat :tools:chunk-compiler:compileChunks
.\harness.cmd
dir assets\maps\chunks    # 0_0.wbc 0_1.wbc 1_0.wbc 1_1.wbc
```
...
client-desktop:
- DesktopLauncher: 1280×720 창, vsync, 제목 "Wildbond". LWJGL3 natives 는 windows-x64 만 의존에 포함 (§5.6). jlink/jpackage(.exe) 는 M3.
- 세이브·로그 경로: %LOCALAPPDATA%\Wildbond\ (System.getenv("LOCALAPPDATA")), 없으면 실행 폴더. Path API 만 사용
client-core:
...
```
.\harness.cmd                   # client-core ArchUnit 포함
.\gradlew.bat :client-desktop:run   # 수용 기준 수동 확인
```
...
```
.\harness.cmd
.\gradlew.bat :client-desktop:run
```
